...
{
    float error = a * b;//operator overloading 
...
template <typename T>
Matrix3<T> Matrix3<T>::operator *(const Matrix3<T> &m) const// what  fucking codes
{
...
template <typename T>
T Matrix3<T>::det() const//calculate the determinant of a matrix
{
...
template <typename T>
void Matrix3<T>::from_axis_angle(const Vector3<T> &v, float theta)//how to convert????
{
    float C = cosf(theta);
    float S = sinf(theta);
    float t = 1.0f - C;
