...
template <typename T>
float wrap_PI(const T radian)//将角度限制在-pi到pi之间
{
    auto res = wrap_2PI(radian);//将角度限制在0到2pi之间
    if (res > M_PI) {
        res -= M_2PI;
    }
    return res;
...
template <typename T>
float wrap_2PI(const T radian)//将角度限制在0到2pi之间
{
    float res = fmodf(static_cast<float>(radian), M_2PI);//%2pi 剩下余数
    if (res < 0) {//保证余数大于0
        res += M_2PI;
    }
    return res;
