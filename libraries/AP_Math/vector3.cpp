...
// dot product 对操作符进行重载，本质上是对类的private部分数据进行一定的操作
//点积 两个向量中对应元素相乘
template <typename T>
...
template <typename T>
Vector3<T> &Vector3<T>::operator /=(const T num)//num 是一个数据
{
    x /= num; y /= num; z /= num;
    return *this;
}
...
//传递引用，但是由于两个变量都声明为const类型，故返回值不能为*this，而是重新定义一个Vector3<T>类型变量
//这些重载中，主调对象本身都声明为const类型，故返回值都是重新声明的一个Vector3类型的变量
//在return语句中声明返回的变量，减少了一个对象声明
template <typename T>
Vector3<T> Vector3<T>::operator -(const Vector3<T> &v) const//num是一个Vector类型，数组相减
{
...
{//this是一个指针，必须采用->   this->length 等效为 (*this).length
