...
float* mat_mul(float *A, float *B, uint8_t n)// N-dimensional matrix multiplication
{//动态创建一个数组并利用memset初始化为零矩阵，new后用delete释放
...
static bool mat_inverse(float* A, float* inv, uint8_t n)
{
    ...
    //free memory 释放new[]申请的动态内存空间，new delete成对出现
    delete[] inv_pivoted;
