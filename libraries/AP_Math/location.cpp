...
#include "AP_Math.h"
#include "location.h"
//你能学到什么?有什么问题?你如何优化此代码?用到什么地方?
//返回的是cos(latitude)
float longitude_scale(const struct Location &loc)//赤道半径6378.2km 
{
    float scale = cosf(loc.lat * 1.0e-7f * DEG_TO_RAD);//GPS直接输出的纬度*1e-7后才是正常纬度
    return constrain_float(scale, 0.01f, 1.0f);
}
//constrain_float(cosf(loc.lat*1.0e-7*DEG_TO_RAD,0.01f,1.0f))


// return distance in meters between two locations
//将两个点的经度投影到第二个点经度所在的纬度圈，如此两点经度在纬度为loc2.lat的纬度圈上
//两点纬度之间的距离在经度圈上计算，两点经度之间的距离在纬度圈上计算
float get_distance(const struct Location &loc1, const struct Location &loc2)
{
    float dlat              = (float)(loc2.lat - loc1.lat);//latitude 两点之间弧度之差 latitude扩大了1.0e7倍
    float dlong             = ((float)(loc2.lng - loc1.lng)) * longitude_scale(loc2);//longitude
    return norm(dlat, dlong) * LOCATION_SCALING_FACTOR;//LOACATION_SCALING_FACTOR 是赤道上1.0e-7度对应的弧长
}
//纬度之间距离:(float)(loc2.lat - loc1.lat) * 1.0e-7 * DEG_TO_RAD * RADICUS_OF_EARTH 即 theta * R
//经度之间距离:(float)(loc2.lng - loc1.lng) * 1.0e-7 * DEG_TO_RAD * (RADICUS_OF_EARTH * cosf(loc2.lat)) 不同纬度圈半径不同
...
/*
  ...
  from loc1 to loc2
 */
 //返回北向、东向两点经纬度之间的距离，米级
Vector2f location_diff(const struct Location &loc1, const struct Location &loc2)
{
    return Vector2f((loc2.lat - loc1.lat) * LOCATION_SCALING_FACTOR,//两点latitude之间距离
                    (loc2.lng - loc1.lng) * LOCATION_SCALING_FACTOR * longitude_scale(loc1));//两点longitude之间距离
}
