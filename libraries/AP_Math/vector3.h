...
template <typename T>//Ä£°åÀà£¬matrixÖÐ±äÁ¿µÄÀàÐÍÊÇ²»¹Ì¶¨µÄ£¬ÓÉÉùÃ÷¶øÈ·¶¨
class Matrix3;
...
public:
    ...
    // trivial ctor ³ÉÔ±³õÊ¼»¯ÁÐ±í½øÐÐ¹¹Ôìº¯ÊýµÄ³õÊ¼»¯£¬ÀàËÆÄ¬ÈÏµÄ¹¹Ôìº¯Êý
    constexpr Vector3<T>()//set the value of parameter x(0) y(0)

        : x(0)
        , y(0)
        , z(0) {}

    // setting ctor Íâ²¿´«Èë²ÎÊý½øÐÐ³ÉÔ±³õÊ¼»¯ÁÐ±íµÄ¹¹Ôìº¯Êý³õÊ¼»¯
    ...
    void operator ()(const T x0, const T y0, const T z0)//operator overloading 
    {
        x= x0; y= y0; z= z0;
    }

    // test for equality
    bool operator ==(const Vector3<T> &v) const;//T is the type of data used in vector3,so the type of formal is Vector3<T>
    ...
    T & operator[](uint8_t i) {
        T *_v = &x;
#if MATH_CHECK_INDEXES
        assert(i >= 0 && i < 3);//¶ÏÑÔ£¬Èç¹û²»Âú×ãµ÷½ÚÔò´òÓ¡Ò»Ìõ³ö´íÐÅÏ¢£¬Ö®ºóÖÕÖ¹³ÌÐòÖ´ÐÐ
#endif
        return _v[i];
    }
    ...
    // zero the vector ½«Vector3ÖÐµÄÔªËØÖÃÁã
    void zero()
    {
        x = y = z = 0;
    }
...
typedef Vector3<int16_t>                Vector3i;//short int Ä£°åÀàÊµÀý»¯µÄ£¬
typedef Vector3<uint16_t>               Vector3ui;//unsigned short int
typedef Vector3<int32_t>                Vector3l;//int 
typedef Vector3<uint32_t>               Vector3ul;//unsigned int
