...
//定义个一个模板类 T是类Matrix中具体的某一个数据类型，比如int、double、float等等
template <typename T>//define a template  T  is type of data ,such as int,char ,float .double ,class and so on
class Matrix3 {
public:

    // Vectors comprising the rows of the matrix 由矩阵的行所组成的向量，a/b/c分别是矩阵的第一、二、三行
    Vector3<T>        a, b, c;
    ...
    // setting ctor
    //同上个一样，利用成员函数初始化列表进行初始化，只不过上面a0都是vector类型，含有三个数据
    //而这种ax、ay只是单个的数据，
    ...
    // function call operator 对矩阵进行赋值
    ...
    // subtraction
    //const 即成员函数运算过程中不修改对象的值
    Matrix3<T> operator        - (const Matrix3<T> &m) const
    {
        return Matrix3<T>(a-m.a, b-m.b, c-m.c);
    }
	//this 是指向对象的指针，*this是对象本身，返回的是对象，但是返回类型可以是引用
    Matrix3<T> &operator        -= (const Matrix3<T> &m)
    {
        return *this = *this - m;
    }
