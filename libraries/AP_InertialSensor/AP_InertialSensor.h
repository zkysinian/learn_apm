 ...
 */
 //默认是私有继承
 //借用Rolfe Schmidt的Gauss-Newton加速度计标定程序
class AP_InertialSensor : AP_AccelCal_Client
{
    friend class AP_InertialSensor_Backend;//友元类
...
public:
    ...
    ///
    const Vector3f     &get_accel(uint8_t i) const { return _accel[i]; }//Vector3f _accel[INS_MAX_INSTANCES];
