...
//gyro * delta_time 
//根据不同的条件有多个return语句，隐式传递对象，将剩余的返回值通过引用传递出去 
//根据数据大小声明所占内存大小 unsigned char
bool AP_InertialSensor::get_delta_angle(uint8_t i, Vector3f &delta_angle) const
{
    if (_delta_angle_valid[i]) {
        ...
        // at higher level
        //get_delta_time 返回一个float类型，故可直接相乘
...
*/
//将得到的速度增量传递给引用输出  将三轴加计输出转换为速度增量 acc * delta_time
//同一个程序中可以根据不同情况有多个return语句
//需要改变的变量用引用方式传递；由于只能返回一个返回值，故可将其他需要返回的值以引用传递出去
//隐式传递const 
...
//声明一个AP的命名空间，该命名空间内有一个返回值为AP_InertialSensor类型的引用，函数名称为ins
namespace AP {
