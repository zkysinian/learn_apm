...
public:
        ...
        _cos_yaw(1.0f)
    {
        ...
        // rate.
        _gyro_drift_limit = AP::ins().get_gyro_drift_rate();//AP命名空间
    ...
    */
    ...
    //_ahrs->get_compass()->last_update_usec()
    ...
    //得到原点位置，将其声明为const，防止被更改
    //结构体Location中主要是latitude longitude height
...
protected:
    ...
    //END导航系下位置的参考点
    struct Location _home;//_home中主要是int型的latitude longitude height
    bool _home_is_set :1;//此种声明方式，占一位
    bool _home_locked :1;
    ...
    uint8_t _active_accel_instance;//typedef unsigned char uint8_t
    ...
    // AOA and SSA
    float _AOA, _SSA;
    uint32_t _last_AOA_update_ms; //typedef unsigned int uint32_t
