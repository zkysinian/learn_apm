...
//若指定的磁力计可被用来计算航向，则返回true
//如果磁状态中可用的标志位1，且学习类型非3，则认为其可用来计算航向
bool Compass::use_for_yaw(uint8_t i) const
{
    ...
    //use_for_yaw是结构体state中磁航向是否可用的标志
    return _state[i].use_for_yaw && _learn.get() != LEARN_INFLIGHT;//LEARN_INFLIGHT = 3 枚举变量
}
...
//数据连续性检查，如何检查 可否用在其他方面
bool Compass::consistent() const
{
    const Vector3f &primary_mag_field = get_field();//得到三轴磁场
    const Vector2f primary_mag_field_xy = Vector2f(primary_mag_field.x,primary_mag_field.y);//得到水平磁场

    if (primary_mag_field_xy.is_zero()) {//若水平磁场强度是0，则返回false
        return false;
    }
    //三轴磁场强度和水平磁场强度归一化
    ...
    for (uint8_t i=0; i<get_count(); i++) {//get_count() 返回磁罗盘的数量
        if (!use_for_yaw(i)) {//如果某个磁罗盘可用来计算航向，则返回true；否则结束本次循环，判断下一个磁力计
            // configured not-to-be-used
            continue;
        }

        Vector3f mag_field = get_field(i);//返回第i个磁力计三轴磁场强度
        Vector2f mag_field_xy = Vector2f(mag_field.x,mag_field.y);//第i个磁力计水平磁场强度
        ...
        if (xyz_ang_diff > AP_COMPASS_MAX_XYZ_ANG_DIFF) {//AP_COMPASS_MAX_XYZ_ANG_DIFF 90°对应的弧度
            return false;
        }

        // check for an unacceptable angle difference on the xy plane
        if (xy_ang_diff > AP_COMPASS_MAX_XY_ANG_DIFF) {//AP_COMPASS_MAX_XY_ANG_DIFF 60°对应的弧度
            return false;
        }

        // check for an unacceptable length difference on the xy plane
        if (xy_len_diff > AP_COMPASS_MAX_XY_LENGTH_DIFF) {//AP_COMPASS_MAX_XY_ANG_DIFF  200.0f
            return false;
        }
    }
    return true;
