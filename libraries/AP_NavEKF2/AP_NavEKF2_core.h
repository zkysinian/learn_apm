...
private:
    ...
    // memory)
    Vector28 statesArray;//真正计算中只有24维，无quat
    struct state_elements {//EKF2的状态变量
        Vector3f    angErr;         // 0..2 角度误差
        Vector3f    velocity;       // 3..5  速度矢量
        Vector3f    position;       // 6..8  位置矢量
        Vector3f    gyro_bias;      // 9..11 陀螺数据偏差
        Vector3f    gyro_scale;     // 12..14陀螺标度因数
        float       accel_zbias;    // 15    z轴加计偏差
        Vector3f    earth_magfield; // 16..18地理系下三轴磁场
        Vector3f    body_magfield;  // 19..21载体系下三轴磁场
        Vector2f    wind_vel;       // 22..23北向、东西风速
        Quaternion  quat;           // 24..27四元数
    } &stateStruct;

    struct output_elements {//EKF2的输出变量
        Quaternion  quat;           // 0..3
    ...
    enum AidingMode {AID_ABSOLUTE=0,    // GPS or some other form of absolute position reference aiding is being used (optical flow may also be used in parallel) so position estimates are absolute.GPS或其他绝对的位置参考辅助被使用，光流也可以并行使用，故位置估计是绝对的
                     AID_NONE=1,       // no aiding is being used so only attitude and height estimates are available. Either constVelMode or constPosMode must be used to constrain tilt drift. 没有辅助被使用，故只有姿态和高度数据是可用的。必须使用constVelMode或constPosMode模式来限制倾斜漂移
                     AID_RELATIVE=2    // only optical flow aiding is being used so position estimates will be relative 只有光流传感器被使用，故位置估计是相对的
                    };
    ...
    //标识在不同的信息融合中方差计算中严重数值误差的标志位
    struct {
        bool bad_xmag:1;//表明bad_xmag占用1位，正常情况下bool占用1个字节，8位
        bool bad_ymag:1;
        bool bad_zmag:1;
        bool bad_airspeed:1;
        bool bad_sideslip:1;
    ...
    //指示哪些GPS质量检查失败的标志
    //正常bool类型占1个字节8位，下面通过结构体的位域概念单独占一位 :1表示该bool类型仅占一位
    struct {
        bool bad_sAcc:1;
        bool bad_hAcc:1;
        bool bad_vAcc:1;
