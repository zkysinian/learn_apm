...
//更新筛选滤波器状态，每当有新的IMU数据可用时，就调用它
void NavEKF2_core::controlFilterModes()
{
    // Determine motor arm status 确定电机状态
...
// return true if we should use the compass 如果使用磁罗盘则返回true
//若所有的磁力计都超时，则不使用磁力计，allMagSensorFailed 返回true
...
//_ahrs->get_compass()返回一个const Compass 类型的指针，故可以接着调用use_for_yaw()成员函数
/*
...
{
    // assume origin at current GPS location (no averaging) 假设原点在目前的GPS定位点，不平均
...
void  NavEKF2_core::updateFilterStatus(void)
{
    ...
    //若指定使用GPS高度，高度被认为是不准确的，直到GPS通过所有检测，
