...
const AP_Param::GroupInfo NavEKF2::var_info[] = {
    ...
    // @Values: 0:Disabled, 1:Enabled 使能EKF2仅用在数学计算中，并不意味着它会被用于飞行控制，如用于飞行控制，要将AHRS_EKF_TYPE = 1，然后重启才会生效
    // @User: Advanced
    // @RebootRequired: True
    ...
    // @Param: GPS_TYPE
    // @DisplayName: GPS mode control  GPS控制模式 0:使用3D速度和2D位置 1:使用2D速度和2D位置(此时对高度估计无作用)   2:使用2D位置 3:禁止GPS使用(gps信号质量较差或多路径效应严重时，此时使用光流)
    ...
    // @Param: VELNE_M_NSE GPS水平测试噪声
    // @DisplayName: GPS horizontal velocity measurement noise (m/s) GPS水平速度测量噪声
    ...
    // @Range: 0.05 5.0   用于设置水平速度观测噪声的下限  如果所用的接收机没有速度精度估计，则使用这个参数。增大它将降低水平量测速度的权重
    // @Increment: 0.05
    // @User: Advanced
    // @Units: m/s
    ...
    // @Param: VELD_M_NSE
    // @DisplayName: GPS vertical velocity measurement noise (m/s) GPS垂直速度噪声
    ...
    // @Param: VEL_I_GATE
    // @DisplayName: GPS velocity innovation gate size GPS速度更新门槛大小
    ...
    // @Range: 100 1000 在速度更新一致性检查中设置标准差的百分比；减小它，有可能抛弃较好的速度量测；增大它，
    // @Increment: 25
    // @User: Advanced
    ...
    // @Param: RNG_USE_SPD
    // @DisplayName: Range finder max ground speed 测距仪的最大地面速度
    ...
    // @Range: 2.0 6.0  当水平速度大于给定值时，测距仪不会用作主要的高度源
    // @Increment: 0.5
    // @User: Advanced
    // @Units: m/s
    AP_GROUPINFO("RNG_USE_SPD", 47, NavEKF2, _useRngSwSpd, 2.0f),

    // @Param: MAG_MASK  使能标识码 用于EKF核心是否使用航向融合
    ...
    // @User: Advanced     bitmask 位掩码 EKF核心的1位位图禁用磁场状态并将在任何场合使用简单的磁航向融合；
    // @RebootRequired: True
    AP_GROUPINFO("MAG_MASK", 48, NavEKF2, _magMask, 0),

    // @Param: OGN_HGT_MASK 掉高参数
...
{
    if (_enable == 0) {//如果EKF2没有使能，就禁止运行下面的EKF流程
        return false;
    }
    const AP_InertialSensor &ins = AP::ins();//获取传感器的数据

    imuSampleTime_us = AP_HAL::micros64();//获取IMU的采样时间

    // remember expected frame time
    _frameTimeUsec = 1e6 / ins.get_sample_rate();//记住预期的帧时间

    //每个预测的IMU帧的具体个数 计算有多少个预测IMU方差，因为有几组IMU，都可以EKF
    ...
    if (core == nullptr) {
        ...
        uint8_t mask = (1U<<ins.get_accel_count())-1;//不要为1个imu运行多个滤波器
        _imuMask.set(_imuMask.get() & mask);
        
        // count IMUs from mask 共有多少个IMU
        num_cores = 0;
        ...
        //如果无法从正常的RAM分配存储空间，尝试从CCM RAm中分配空间
        ...
        for (uint8_t i = 0; i < num_cores; i++) {
            //Call Constructors 创建对象用来做EKF运算，
            //这里创建两个对象，对应两组IMU，为后面优先选择作铺垫，达到IMU冗余的效果
        ...
        for (uint8_t i=0; i<7; i++) {
            if (_imuMask & (1U<<i)) {
				//重要 初始化不同的IMU
                if(!core[num_cores].setup_core(this, i, num_cores)) {//初始化传感器
                    return false;
                }
                num_cores++;
            }
    ...
    bool ret = true;
    for (uint8_t i=0; i<num_cores; i++) {
        ret &= core[i].InitialiseFilterBootstrap();//初始化传感器引导
    }
    //使用捕获重置事件的结构,用memset将元素置零
...
//更新滤波器状态 只要新的可用的IMU数据到来，该函数被调用
void NavEKF2::UpdateFilter(void)
{
    if (!core) {//如果没有为IMU对象分配内核，则退出
        return;
    }

    imuSampleTime_us = AP_HAL::micros64();//获取imu采样时间
    
    const AP_InertialSensor &ins = AP::ins();//获取IMU数据

    bool statePredictEnabled[num_cores];//两个内核
    //因为前面有两个内核，所以这里对所有满条件的IMU都进行EKF处理，num_scres = 2
    for (uint8_t i=0; i<num_cores; i++) {
        ...
        if (core[i].getFramesSincePredict() < (_framesPerPrediction+3) &&
            ...
            statePredictEnabled[i] = false;//最后进入这里
        } else {
            statePredictEnabled[i] = true;
        }
        core[i].UpdateFilter(statePredictEnabled[i]);//比较重要的函数
    }
