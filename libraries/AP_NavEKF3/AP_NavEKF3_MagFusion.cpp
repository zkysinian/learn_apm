...
void NavEKF3_core::SelectMagFusion()
{
    ...
    // check for and read new magnetometer measurements  注意主要是check
    readMagData();
    ...
    // check for availability of magnetometer data to fuse 融合前检查磁罗盘数据是否可用
...
*/
//采用matlab符号工具箱产生的代数方程进行磁航向数据融合
//当外部磁场环境受到干扰(靠近金属物体或地面)，也是可用的
//当水平面磁场强度微弱(距离磁极在30度之内)不可用的
void NavEKF3_core::fuseEulerYaw()
{
    ...
    // compass measurement error variance (rad^2) R 阵
