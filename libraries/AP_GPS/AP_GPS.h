...
public:
   ...
   enum GPS_Config {
    ...
    //返回主传感器的索引，该传感器是有助于输出的传感器索引。
    //混合解决方案可作为附加的实例加以使用
    uint8_t primary_sensor(void) const {
        return primary_instance;
    }

    /// Query GPS status 询问GPS的状态，有多个GPS
    GPS_Status status(uint8_t instance) const {
        return state[instance].status;
    }
	//询问主GPS的状态  status为一个枚举类型，存放一些枚举定义
